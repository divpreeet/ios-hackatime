...
struct ProfileView: View {
    ...
    func loadAll() async {
        await MainActor.run { error = nil; loading = true }
        let key = apiKey.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !key.isEmpty else {
            await MainActor.run { error = "Enter an API Key"; loading = false }
            return
        }
        ...
        await MainActor.run { loading = false; lastRefresh = Date() }
    }
    
    func saveCache() {
        if let stats, let data = try? JSONEncoder().encode(stats) {
            UserDefaults.standard.set(data, forKey: "cachedStats")
        }
        if let data = try? JSONEncoder().encode(editorStats) {
            UserDefaults.standard.set(data, forKey: "cachedEditorStats")
        }
        UserDefaults.standard.set(recentP, forKey: "cachedRecentP")
        if let trustF, let data = try? JSONEncoder().encode(trustF) {
            UserDefaults.standard.set(data, forKey: "cachedTrustF")
        }
        UserDefaults.standard.set(todayS, forKey: "cachedTodayS")
        UserDefaults.standard.set(Date().timeIntervalSince1970, forKey: "lastRefresh")
    }
    
    func loadCache() {
        if let data = UserDefaults.standard.data(forKey: "cachedStats"),
           let statsLoaded = try? JSONDecoder().decode(UserStats.self, from: data) {
            stats = statsLoaded
        }
        if let data = UserDefaults.standard.data(forKey: "cachedEditorStats"),
           let editorsLoaded = try? JSONDecoder().decode([EditorStat].self, from: data) {
            editorStats = editorsLoaded
        }
        recentP = UserDefaults.standard.string(forKey: "cachedRecentP") ?? "-"
        if let data = UserDefaults.standard.data(forKey: "cachedTrustF"),
           let trustLoaded = try? JSONDecoder().decode(trustFactor.self, from: data) {
            trustF = trustLoaded
        }
        todayS = UserDefaults.standard.integer(forKey: "cachedTodayS")
    ...
    func clearCache() {
        UserDefaults.standard.removeObject(forKey: "cachedStats")
        UserDefaults.standard.removeObject(forKey: "cachedEditorStats")
        UserDefaults.standard.removeObject(forKey: "cachedRecentP")
        UserDefaults.standard.removeObject(forKey: "cachedTrustF")
        UserDefaults.standard.removeObject(forKey: "cachedTodayS")
